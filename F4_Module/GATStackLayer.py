...
'''
@Time: 16:56
@FileName: GATStackLayer.py
@Author: Zhao Qi-ao
'''
import torch
import torch.nn as nn

from F4_Module.GATLayer import SGATLayer
...
#avg的话就不用将out_dim除以head数目之后要求可以整除了
class MultiavgLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, attn_drop_out, feat_embed_size, layer, merge='avg'):
        super(MultiavgLayer, self).__init__()
        self.heads = nn.ModuleList()
        for i in range(num_heads):
            self.heads.append(layer(in_dim, out_dim, feat_embed_size))  # [n_nodes, hidden_size]
        self.merge = merge
    ...
    def forward(self, g, h):
        ...
        if self.merge == 'cat':
            ...
            # merge using average
            result = torch.mean(torch.stack(head_outs),dim=0)
        return result
