...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        assert not torch.any(torch.isnan(x)), "FFN input is problem"
        residual = x
...
class WGATLayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        wedge_id = g.filter_edges(lambda edges: edges.data["dtype"] == 1)
        z = self.fc(h)
...
class SNGATLayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        snode_id = g.filter_nodes(lambda nodes: nodes.data["unit"] == 1)
        snedge_id = g.filter_edges(lambda edges: (edges.src["dtype"] == 1) & (edges.dst["dtype"] == 2))
        z = self.fc(h)
        g.nodes[snode_id].data['z'] = z
        g.apply_edges(self.edge_attention, edges=snedge_id)
        g.pull(Nnode_id, self.message_func, self.reduce_func)
        g.ndata.pop('z')
        h = g.ndata.pop('sh')
