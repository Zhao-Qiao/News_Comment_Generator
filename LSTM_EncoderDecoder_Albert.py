...
'''
@Time: 22:42
@FileName: LSTM_EncoderDecoder_Albert.py
@Author: Zhao Qi-ao
'''
import random
...
class lstm_decoder(nn.Module):
    ...
    def forward(self, x,state):
        ...
        lstm_output, state= self.lstm(torch.cat((x,context), 2) , state)
...
class S2sLSTM(nn.Module):
    ...
    def forward(self, src_ids, back_tgt_ids, for_tgt_ids, src_pad_mask=None, back_tgt_pad_mask=None,
        ...
        senti_memory = torch.repeat_interleave(senti_memory.unsqueeze(0), repeats=state[-1].shape[0], dim=0)
        memory = self.memory_linear(torch.concat((state[-1], senti_memory), 2))
