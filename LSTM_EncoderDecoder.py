...
'''
@Time: 17:38
@FileName: transformerEncoderDecoder.py
@Author: Zhao Qi-ao
'''
import random
...
class lstm_encoder(nn.Module):
    ...
    def forward(self, input):
        lstm_output, state = self.lstm(input.view(input.shape[0], input.shape[1], self.input_size))
...
class lstm_decoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1):
        ...
        self.input_size = input_size
        self.hidden_size= hidden_size
        self.lstm = nn.LSTM(input_size=self.input_size + self.hidden_size, hidden_size=self.hidden_size, num_layers=num_layers) # concat hidden state and vocab embedding
        self.linear= nn.Linear(self.hidden_size, self.input_size)

    def forward(self, x,state):
        '''
        :param x: input sequence
        :param hidden: last state passed from encoder
        :return: output, state
        '''
        context = state[-1][-1].repeat(x.shape[0],1,1) # use the hidden state of the last layer of the encoder as the context
        lstm_output, state= self.lstm(torch.cat((x,context), 2) , state) # concatenate input and context, and initial cell state by the last state of the encoder
        output = self.linear(lstm_output) # output layer, map from hidden size to word embedding dim
        return output, state



class S2sLSTM(nn.Module):
    def __init__(self, vocab, senti_model=None, word_emb_dim=128, nhead=4, pretrained_weight=None,
        ...
        # Encoder
        self.encoder = lstm_encoder(input_size=word_emb_dim, hidden_size=self.model_dim, num_layers=3)
        self.memory_linear = nn.Linear( word_emb_dim, word_emb_dim)
        self.memory_linear_dropout = nn.Dropout(p=0.1)
        self.for_decoder = lstm_decoder(input_size=word_emb_dim, hidden_size=self.model_dim, num_layers=3)
        self.back_decoder = lstm_decoder(input_size=word_emb_dim, hidden_size=self.model_dim, num_layers=3)
        # 输出层
    ...
    def forward(self, src_ids, back_tgt_ids, for_tgt_ids, src_pad_mask=None, back_tgt_pad_mask=None,
        ...
        # state[-1] = memory
        for_outputs,_ = self.for_decoder(for_tgt, state)
        back_outputs,_ = self.back_decoder(back_tgt, state)
